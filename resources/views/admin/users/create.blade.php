        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="name" id="name" required placeholder="{{ __('admin/users.name') }}" maxlength="191" value="{{ old('name') }}">
                </div>
                <div class="form-group">
                    <label for="username">{{ __('admin/users.username') }} (*)</label>
                    <input type="text" class="form-control" name="username" id="username" required placeholder="{{ __('admin/users.username') }}" maxlength="191" value="{{ old('username') }}">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="email" class="form-control" name="email" id="email" placeholder="{{ __('admin/users.email') }}" maxlength="191" value="{{ old('email') }}">
                </div>
                <div class="form-group">
                    <label for="mobile">{{ __('admin/users.mobile') }}</label>
                    <input type="text" class="form-control" name="mobile" id="mobile" placeholder="{{ __('admin/users.mobile') }}" maxlength="191" value="{{ old('mobile') }}">
                </div>
                <div class="custom-control custom-checkbox">
                    <input type="checkbox" @if (old('active', 1) == 1) checked @endif class="custom-control-input" name="activate" id="activate" onclick="$('#active').val($(this).prop('checked') ? '1' : '0');">
                    <label class="custom-control-label" for="activate">{{ __('admin/users.activated') }}</label>
                    <input hidden type="text" name="active" id="active" value="{{ old('active', 1) }}">
                </div>
            </div>
        </div>
    </div>
