...
<nav class="navbar navbar-light bg-white shadow">
    <div class="navbar-brand">{{ __('admin/users.lists') }}</div>
    <div class="mr-auto">
    ...
    <table id="table-data" class="table table-hover">
        <thead>
            <tr>
                ...
                <th scope="col">{{ __('admin/users.username') }}</th>
                <th scope="col">{{ __('admin/users.email') }}</th>
                <th scope="col">{{ __('admin/users.active') }}</th>
        ...
        <tbody id="data">
            ...
            <tr>
                ...
                <td>{{ $data->username }}</td>
                <td>{{ $data->email }}</td>
                <td>{{ $data->active ? __('admin/users.activated') : __('admin/users.deactivated') }}</td>
                @if (Route::has($route . '.show'))
                <td class="px-1">
                    <a href="{{ route($route . '.show', ['user' => $data]) }}" class="btn btn-info" title="{{ __('admin/table.update') }}">
                ...
                @if (Route::has($route . '.edit'))
                <td class="px-1">
                    <a href="{{ route($route . '.edit', ['user' => $data]) }}" class="btn btn-primary" title="{{ __('admin/table.update') }}">
                ...
                @if (Route::has($route . '.destroy'))
                <td class="px-1">
                    <div data-toggle="modal" data-target="#cfmDel" onclick="$('#formDelete').attr('action', '{{ route($route . '.destroy', ['user' => $data]) }}');" class="btn btn-danger" title="{{ __('admin/table.delete') }}">
