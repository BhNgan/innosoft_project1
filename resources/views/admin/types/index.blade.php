...
<nav class="navbar navbar-light bg-white shadow">
    <div class="navbar-brand">{{ __('admin/types.list') }}</div>
    <div class="mr-auto">
...
<div class="bg-white div-content my-3 p-3 table-responsive shadow">
    <table id="table-data" class="table table-hover">
        <thead>
            <tr>
                <th scope="col">#</th>
                <th scope="col">{{ __('admin/types.avatar') }}</th>
                <th scope="col">{{ __('admin/types.type_name') }}</th>
                <th scope="col">{{ __('admin/types.sort') }}</th>
                <th scope="col">{{ __('admin/types.note') }}</th>
        ...
        <tbody id="data">
            ...
            <tr>
                ...
                <td><img src="{{ $data->avatar ? asset($data->avatar) : "" }}" height="40" class="rounded"></td>
                <td>{{ $data->type_name }}</td>
                <td>{{ $data->sort }}</td>
                <td>{{ $data->note }}</td>
                @if (Route::has($route . '.show'))
                <td class="px-1">
                    <a href="{{ route($route . '.show', ['type' => $data]) }}" class="btn btn-info" title="{{ __('admin/table.update') }}">
                ...
                @if (Route::has($route . '.edit'))
                <td class="px-1">
                    <a href="{{ route($route . '.edit', ['type' => $data]) }}" class="btn btn-primary" title="{{ __('admin/table.update') }}">
                ...
                @if (Route::has($route . '.destroy'))
                <td class="px-1">
                    <div data-toggle="modal" data-target="#cfmDel" onclick="$('#formDelete').attr('action', '{{ route($route . '.destroy', ['type' => $data]) }}');" class="btn btn-danger" title="{{ __('admin/table.delete') }}">
