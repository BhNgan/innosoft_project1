...
@section('content')
<form method="POST" action="{{ route($route . '.update', ['type' => $data]) }}" role="form" enctype="multipart/form-data">
@method('PUT')
@csrf
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="type_name" id="type_name" required placeholder="{{ __('admin/types.type_name') }}" maxlength="191" value="{{ old('type_name', $data->type_name) }}">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="number" class="form-control" name="sort" id="sort" required placeholder="{{ __('admin/types.sort') }}" maxlength="191" value="{{ old('sort', $data->sort) }}">
                </div>
                <div class="form-group">
                    <label for="note">{{ __('admin/types.note') }}</label>
                    <input type="text" class="form-control" name="note" id="note" placeholder="{{ __('admin/types.note') }}" maxlength="191" value="{{ old('note', $data->note) }}">
                </div>
            </div>
        </div>
    </div>
